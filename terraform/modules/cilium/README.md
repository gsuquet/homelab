...
Installs [Cilium](https://cilium.io/) as the cluster's CNI via its official Helm chart, providing CNI networking, eBPF-based kube-proxy replacement, network policies, transparent pod-to-pod encryption, and Hubble observability.
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_bandwidth_manager_bbr"></a> [bandwidth\_manager\_bbr](#input\_bandwidth\_manager\_bbr) | Activate BBR TCP congestion control for Pods (requires bandwidth\_manager\_enabled = true). | `bool` | `true` | no |
| <a name="input_bandwidth_manager_enabled"></a> [bandwidth\_manager\_enabled](#input\_bandwidth\_manager\_enabled) | Enable Cilium's bandwidth manager infrastructure for EDT (Earliest Departure Time) packet pacing and Pod bandwidth enforcement. | `bool` | `true` | no |
...
| <a name="input_kube_proxy_replacement"></a> [kube\_proxy\_replacement](#input\_kube\_proxy\_replacement) | Replace kube-proxy with Cilium's eBPF datapath (fewer moving parts, faster L3-L4 enforcement). | `bool` | `true` | no |
...
| <a name="input_policy_enforcement_mode"></a> [policy\_enforcement\_mode](#input\_policy\_enforcement\_mode) | Cilium network policy enforcement mode: 'default' (deny only for endpoints selected by a policy), 'always' (deny-by-default for every endpoint), or 'never'. | `string` | `"default"` | no |
